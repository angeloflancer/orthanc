...
router.post('/upload', protect, checkFeatureAccess(), upload.single('file'), async (req, res) => {
  try {
    ...
    const wordFile = wordFileRepo.create({
    ...
    let patient = patientRepo.findOne({ patientId: patientId.trim() });
    if (!patient) {
      ...
      const updates = { wordFileCount: (patient.wordFileCount || 0) + 1 };
      if (!patient.patientName && patientName) updates.patientName = patientName.trim();
...
const { getUserHospital } = require('../utils/hospitalHelper');
...
router.get('/', protect, checkFeatureAccess(), async (req, res) => {
  try {
    ...
    if (req.query.hospital && req.query.hospital.trim()) {
      const hospitals = hospitalRepo.find({ nameSearch: req.query.hospital.trim() });
      query.hospitalIds = hospitals.map(h => h.id);
      if (query.hospitalIds.length === 0) {
        return res.json({ success: true, wordFiles: [], pagination: { page, limit, total: 0, pages: 0 } });
      }
    }
    const total = wordFileRepo.countDocuments(query);
    const wordFiles = wordFileRepo.find(query, { limit, skip });
    res.json({
      success: true,
      wordFiles: wordFiles.map(file => ({
        ...
        hospitalName: file.hospital ? (hospitalRepo.findById(file.hospital) || {}).name || '' : '',
...
router.get('/:id', protect, checkFeatureAccess(), async (req, res) => {
  try {
    const wordFile = wordFileRepo.findById(req.params.id);
    if (!wordFile) return res.status(404).json({ error: 'Word file not found' });
    const user = getRequestUser(req);
    if (!user) return res.status(401).json({ error: 'User not found' });
    res.json({
      success: true,
...
router.delete('/:id', protect, checkFeatureAccess(), async (req, res) => {
  try {
    ...
    const patient = patientRepo.findOne({ patientId: wordFile.patientId });
    if (patient) {
      patientRepo.update(patient.id, { wordFileCount: Math.max(0, (patient.wordFileCount || 0) - 1) });
    }
    wordFileRepo.deleteById(req.params.id);
    res.json({ success: true, message: 'Word file deleted successfully' });
  } catch (error) {
